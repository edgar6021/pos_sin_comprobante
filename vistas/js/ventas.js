...
function listarMetodos() {
	let metodo = $("#nuevoMetodoPago").val();
	let listaMetodos = [];

	if (metodo === "Efectivo") {
		let monto = parseFloat($("#nuevoValorEfectivo").val() || 0);
		let total = parseFloat($("#totalVenta").val() || 0);

		if (monto < total) {
			swal({
				title: "El monto en efectivo es insuficiente",
				text: "Debe ser igual o mayor al total de la venta",
				type: "error",
				confirmButtonText: "¡Cerrar!"
			});
			throw new Error("El monto en efectivo es insuficiente");
		}
		...
		let transaccion = $("#nuevoCodigoTransaccion").val();
		let total = $("#totalVenta").val();

		if (!transaccion) {
			swal({
				title: "Código de transacción requerido",
				text: "Debe ingresar un código válido",
				type: "error",
				confirmButtonText: "¡Cerrar!"
			});
			throw new Error("Código de transacción faltante");
		}
...
$(".formularioVenta").on("submit", function (e) {
	try {
		...
		e.preventDefault(); 
		//console.warn("🛑 Envío cancelado por validación:", error.message);
	}
});
